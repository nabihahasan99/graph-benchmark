"""
Load IMDb graph data into Memgraph Cloud
Uses Cypher queries via Neo4j driver (Memgraph is Cypher-compatible)
"""
...
class MemgraphLoader:
    def __init__(self):
        ...
        self.user = os.getenv('MEMGRAPH_USER', 'cognodb')
        self.password = os.getenv('MEMGRAPH_PASSWORD')

        if not all([self.uri, self.password]):
            raise ValueError("Missing Memgraph credentials in .env file")

        print(f"Connecting to Memgraph...")
    ...
    def save_stats(self, output_path):
        ...
        self.stats['database'] = 'Memgraph Cloud'
...
def main():
    ...
    print("\n" + "="*60)
    print("Memgraph data loading complete!")
    print("="*60)
